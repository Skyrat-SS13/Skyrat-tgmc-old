...
/obj/item/attack_hand(mob/living/user)
	...
	var/remove_successful = TRUE // SKYRAT ADDITION
	if(istype(loc, /obj/item/storage))
		var/obj/item/storage/S = loc
		remove_successful = S.remove_from_storage(src, user.loc) // SKYRAT ADDITION
	...
	pickup(user)
	if(!user.put_in_active_hand(src) || !remove_successful) // SKYRAT EDIT - ORIGINAL: if(!user.put_in_active_hand(src))
